...
class UserscontrolCommon
{
	...
	function create_plugin_setting($type, $id, $label, $pairs, $help, $inline_help = '', $extra=null) {
		...
		echo wp_kses($cmp, $userscontrol->allowed_html) ;
        ...
        switch ($type) {
            ...
            case 'textarea':
			  ...
			  case 'textarearich':			  
				...
				$html = $userscontrol->user->get_me_wphtml_editor($id, $value);				
        ...
        if (is_array($extra)) {
			echo  wp_kses('<div class="helper-wrap">', $userscontrol->allowed_html) ;
            foreach ($extra as $a) {
                echo wp_kses($a, $userscontrol->allowed_html);
            }
			echo wp_kses("</div>",  $userscontrol->allowed_html);
        }       	
		echo wp_kses("</td></tr>",  $userscontrol->allowed_html);
    }
...
$key = "commmonmethods";
$this->{$key} = new UserscontrolCommon();
?>